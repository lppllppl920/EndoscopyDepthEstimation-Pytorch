...
if __name__ == '__main__':
    ...
    log_root = Path(
        evaluation_result_root) / "depth_estimation_evaluation_sfmlearner_run_{}_{}_{}_{}_test_id_{}".format(
        currentDT.month,
        currentDT.day,
        currentDT.hour,
        currentDT.minute,
    ...
    if phase == "validation":
        ...
        with torch.no_grad():
            ...
            for batch, (
                ...
                predicted_depth_maps_1 = 1.0 / depth_estimation_model(colors_1)
                predicted_depth_maps_2 = 1.0 / depth_estimation_model(colors_2)
                ...
                color_array = cv2.cvtColor(
                    np.uint8(255 * (colors_1[0].permute(1, 2, 0).data.cpu().numpy() + 1.0) * 0.5),
                    cv2.COLOR_RGB2BGR)
                ...
                colors_1_display, sparse_depths_1_display, pred_depths_1_display, warped_depths_1_display, sparse_flows_1_display, dense_flows_1_display = \
                    utils.display_color_sparse_depth_dense_depth_warped_depth_sparse_flow_dense_flow(idx=1, step=batch,
                                                                                                     writer=writer,
                                                                                                     colors_1=colors_1,
                ...
                colors_2_display, sparse_depths_2_display, pred_depths_2_display, warped_depths_2_display, sparse_flows_2_display, dense_flows_2_display = \
                    utils.display_color_sparse_depth_dense_depth_warped_depth_sparse_flow_dense_flow(idx=2, step=batch,
                                                                                                     writer=writer,
                                                                                                     colors_1=colors_2,
        ...
        with torch.no_grad():
            ...
            for batch, (colors_1, boundaries, intrinsics, names) in enumerate(test_loader):
                ...
                color_display = (colors_1[0].permute(1, 2, 0) + 1.0) * 0.5 * boundaries[0].view(height, width, 1)
                color_display = np.uint8(255 * color_display.data.cpu().numpy())
                ...
                depth_display = cv2.applyColorMap(
                    np.uint8(255 * np.log(1.0 + depth_map) / np.max(np.log(1.0 + depth_map))), cv2.COLORMAP_JET)
