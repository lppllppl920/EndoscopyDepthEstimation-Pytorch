...
if __name__ == '__main__':
    ...
    log_root = Path(evaluation_result_root) / "evaluation_depth_estimation_run_{}_{}_{}_{}_bag_{}".format(
        currentDT.month,
        currentDT.day,
        currentDT.hour,
        currentDT.minute,
    ...
    if phase == "validation":
        test_dataset = dataset.SfMDataset(image_file_names=test_filenames,
                                          ...
                                          num_pre_workers=num_workers, visible_interval=30,
                                          rgb_mode="rgb")
        ...
        with torch.no_grad():
            ...
            for batch, (colors_1, boundaries, intrinsics, names) in enumerate(test_loader):
                ...
                color_display = cv2.cvtColor(
                    np.uint8(255 * color_display.data.cpu().numpy()),
                    cv2.COLOR_RGB2BGR)
