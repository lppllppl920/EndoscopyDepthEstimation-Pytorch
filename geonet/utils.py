...
def get_test_color_img(img_file_name, start_h, end_h, start_w, end_w, downsampling_factor, is_hsv=False):
...
def get_pair_color_imgs(prefix_seq, pair_indexes, start_h, end_h, start_w, end_w, downsampling_factor, is_hsv=False):
    imgs = []
...
def euler2mat(z, y, x):
    ...
    ones = torch.ones(batch_size).float().cuda()
    zeros = torch.zeros(batch_size).float().cuda()
...
def pose_vec2mat(vec):
    ...
    fill = torch.tensor([0, 0, 0, 1]).type(
        torch.FloatTensor).view(1, 4).repeat(batch_size, 1, 1).cuda()
    # shape: (#batch,4,4)
...
def meshgrid(height, width, is_homogeneous=True):
    global device
    x = torch.ones(height).float().view(height, 1).cuda()
    # shape : (h,w)
    x = torch.matmul(x, torch.linspace(0, 1, width).view(1, width).cuda())

    y = torch.linspace(0, 1, height).view(height, 1).cuda()
    # shape : (h,w)
    y = torch.matmul(y, torch.ones(width).float().view(1, width).cuda())

    x = x * (width - 1)
    y = y * (height - 1)

    if is_homogeneous:
        ones = torch.ones(height, width).float().cuda()
...
def get_data_balancing_scale(poses, visible_view_count):
    ...
    pre_translation = np.zeros((3,), dtype=np.float)
    for i in range(visible_view_count):
...
def display_output(phase, idx, step, writer, colors_1, scaled_depth_maps_1, is_hsv):
    colors_display = vutils.make_grid(colors_1 * 0.5 + 0.5, normalize=False)
    colors_display = np.moveaxis(colors_display.data.cpu().numpy(),
                                 source=[0, 1, 2], destination=[2, 0, 1])
    ...
    if is_hsv:
        colors_display = cv2.cvtColor(colors_display, cv2.COLOR_HSV2RGB_FULL)
    writer.add_image(phase + '/Images/Color_' + str(idx),
                     np.moveaxis(colors_display, source=[0, 1, 2], destination=[1, 2, 0]), step)
    ...
    return colors_display, depths_display
