...
def pre_processing_data(process_id, folder_list, downsampling, network_downsampling, suggested_h,
                        suggested_w, queue_intrinsic_matrix, queue_mask_boundary, queue_selected_indexes,
    ...
    for folder in folder_list:
        ...
        cropped_downsampled_undistorted_mask_boundary /= 255.0
        cropped_downsampled_undistorted_mask_boundary[cropped_downsampled_undistorted_mask_boundary < 0.9] = 0.0
        cropped_downsampled_undistorted_mask_boundary[cropped_downsampled_undistorted_mask_boundary >= 0.9] = 1.0
...
class EndoscopyFolder(Dataset):
    def __init__(self, image_file_names, folder_list, adjacent_range=(1, 10),
                 transform=None, downsampling=1.0, network_downsampling=64, use_store_data=False, store_data_root=None,
                 phase="train", sequence_length=3, pre_processing_workers=12):
        ...
        self.workers = pre_processing_workers
        ...
        if not use_store_data:
            ...
            interval = len(self.folder_list) // self.workers
            ...
            for i in range(self.workers - 1):
                process_pool.append(Process(target=find_largest_size, args=(
                    ...
                    queue_size)))
            process_pool.append(Process(target=find_largest_size, args=(
                self.folder_list[(self.workers - 1) * interval:], self.downsampling, self.network_downsampling,
                queue_size)))
            ...
            for i in range(self.workers - 1):
                process_pool.append(Process(target=pre_processing_data,
                                            args=(i, self.folder_list[i * interval: (i + 1) * interval],
                                                  self.downsampling, self.network_downsampling, largest_h, largest_w,
                                                  queue_intrinsic_matrix, queue_mask_boundary,
                                                  queue_selected_indexes,
                                                  queue_crop_positions)))
            process_pool.append(Process(target=pre_processing_data,
                                        args=(self.workers - 1, self.folder_list[(self.workers - 1) * interval:],
                                              self.downsampling, self.network_downsampling, largest_h, largest_w,
                                              queue_intrinsic_matrix, queue_mask_boundary,
                                              queue_selected_indexes,
                                              queue_crop_positions)))
            ...
            with open(str(store_data_root / ("train" + "_" + str(
                    self.downsampling) + "_" + str(self.network_downsampling) + ".pkl")), "wb") as f:
                pickle.dump(
                    [self.crop_positions_per_seq, self.selected_indexes_per_seq,
                     self.intrinsic_matrix_per_seq,
                     self.mask_boundary_per_seq, self.view_indexes_per_point_per_seq,
                     self.downsampling, self.network_downsampling],
                    f, pickle.HIGHEST_PROTOCOL)
        else:
            with open(str(store_data_root / ("train" + "_" + str(
                    self.downsampling) + "_" + str(self.network_downsampling) + ".pkl")), "rb") as f:
                [self.crop_positions_per_seq, self.selected_indexes_per_seq,
                 self.intrinsic_matrix_per_seq,
                 self.mask_boundary_per_seq, self.view_indexes_per_point_per_seq,
                 self.downsampling, self.network_downsampling] = pickle.load(f)
    ...
    def __getitem__(self, idx):
        if self.phase != "test":
            ...
            while True:
                ...
                if increment not in increment_list:
                    increment_list.append(increment)
                    frame_count += 1
                    if frame_count >= self.sequence_length + 1:
                        break

            group_indexes = [self.selected_indexes_per_seq[folder][pos]]
            for increment in increment_list:
                group_indexes.append(self.selected_indexes_per_seq[folder][pos + increment])

            group_imgs = utils.get_pair_color_imgs(prefix_seq=folder, pair_indexes=group_indexes, start_h=start_h,
                                                   start_w=start_w,
                                                   end_h=end_h, end_w=end_w, downsampling_factor=self.downsampling,
                                                   is_hsv=False)

            tgt_img = group_imgs[0]
            tgt_img = tgt_img.astype(np.float32).reshape((tgt_img.shape[0], tgt_img.shape[1], 3))
            ref_imgs = [ref_img.astype(np.float32).reshape((ref_img.shape[0], ref_img.shape[1], 3)) for ref_img in
                        group_imgs[1:]]

            mask = self.mask_boundary_per_seq[folder]
            mask = mask.astype(np.float32).reshape((mask.shape[0], mask.shape[1], 1))
            print("mask {}".format(np.max(mask)))
            if self.transform is not None:
                imgs, mask, intrinsics = self.transform([tgt_img] + ref_imgs, [mask], intrinsics)
                tgt_img = imgs[0]
                ref_imgs = imgs[1:]

            tgt_img = self.normalize(tgt_img)
            ref_imgs = torch.cat([self.normalize(img) for img in ref_imgs], dim=2)

            return img_to_tensor(tgt_img), img_to_tensor(ref_imgs), img_to_tensor(mask[0]), torch.from_numpy(
                intrinsics), torch.from_numpy(np.linalg.inv(intrinsics))
            ...
            training_color_img_1 = utils.get_test_color_img(img_file_name, start_h, end_h, start_w, end_w,
                                                            self.downsampling, is_hsv=False)
            ...
            training_mask_boundary = self.mask_boundary_per_seq[folder].astype("float32")
            training_mask_boundary = training_mask_boundary.reshape(
                (training_mask_boundary.shape[0], training_mask_boundary.shape[1], 1))

            training_color_img_1 = self.normalize(training_color_img_1)
            return img_to_tensor(training_color_img_1), img_to_tensor(training_mask_boundary), \
                   torch.from_numpy(training_intrinsic_matrix), img_file_name[-12:-4]
